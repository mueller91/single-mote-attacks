...
def apply_debug_flags(contiki, new_old_pair='NONE', logger=None):
    ...
    for filename in DEBUG_FILES:
        ...
        logger.debug(" > Replacing in {} where regex {} with new value: {}".format(filename, old_pattern, new_old_pair))
        replace_in_file(join(contiki, filename), (old_pattern, new_old_pair), logger=logger)
