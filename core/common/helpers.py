...
from os.path import exists, expanduser, join, split, basename, dirname
...
def replace_in_file(path, replacements, logger=None):
            ...
            for line in of.readlines():
                for replacement, regex in zip(replacements, regexs):
                    ...
                    if replacement[0] in line:
                        ...
                        replacements_found.append((replacement[0], replacement[1]))
                        ...
                        if regex is not None:
                            ...
                            if match is not None:
                                try:
                                    ...
                                    replacements_found.append((match.groups(0)[0], replacement[1]))
                                except IndexError:
                                    line = line.replace(match.group(), replacement[1])
                                    replacements_found.append(([match.group()], replacement[1]))
                                break
    ...
    if logger:
        ...
        for k in c.keys():
            logger.debug(" > Found and replaced \"{}\" with \"{}\" {} times in path: {}".format(k[0], k[1], c[k], path))
