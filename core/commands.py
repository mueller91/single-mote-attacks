...
from core import *
from core.common.helpers import __expand_folders
from core.common.wsngenerator import relocate_malicious_mote
...
def __make(name, ask=True, **kwargs):
    ...
    :param kwargs: simulation keyword arguments (see the documentation for more information)
    """
    # ================ Set up needed variabled and check kwargs
    global reuse_bin_path
    set_logging(kwargs.get('loglevel'))
    # gets test data path, e.g.: '/home/mueller/RPL-Attacks-Data/8/control-set-clean'
    path = kwargs['path']
    # kwargs is a dictionary, containing information from rpl-attacks.json plus other stuff, such as node positions
    ...
    # templates is e.g.: '/home/mueller/RPL-Attacks-Data/8/control-set-clean/templates'
    templates = get_path(path, 'templates', create=True)
    # select the right malicious mote template and duplicate the simulation file
    logger.debug(" > Copying necessary files from {} to {}".format(__expand_folders((TEMPLATES_FOLDER, 'experiment')),
                                                                   __expand_folders(templates)))
    # import json
    ...
    files_to_copy = [
        ...
        'Makefile', 'simulation.csc', 'script.js', 'report.md',
        "motes/Makefile" if "custom_makefile" in kwargs.keys() else ("motes/old_makefile/Makefile", "motes/Makefile")
    ]
    additional_files = [(x["src"], x["dst"]) for x in
                        kwargs["additional_files"]] if "custom_makefile" in kwargs.keys() else []
    files_to_copy.extend(additional_files)

    # ===================== FROM RPL-ATTACKS TO TARGET FOLDER
    # TEMPLATES_FOLDER is '/home/mueller/spielwiese/rpl-attacks/templates'
    # templates is '/home/mueller/RPL-Attacks-Data/8/control-set-clean/templates'
    # before, 8/ is empty, after, 8/ contains control-set-clean/templates
    copy_files((TEMPLATES_FOLDER, 'experiment'), templates, *files_to_copy)
    logger.debug(" > Type of Motes: Root: {}, Sensor: {}, Malicious: {}".format(
        params["mtype_root"], params["mtype_sensor"], params["mtype_malicious"]))
    # create experiment's files from templates then clean the templates folder
    # This effectively copies from template folder to data folder
    replacements = render_templates(path, **params)
    # copy add files. dst->dst is actually correct, since files have already been copied once and we just want to keep
    # their structure when copying them again
    copy_files(join(path, "templates"), join(path, "with-malicious"),
               *[(x["dst"], x["dst"]) for x in
                 kwargs["additional_files"]] if "custom_makefile" in kwargs.keys() else [])
    ...
    with settings(hide(*HIDDEN_ALL), warn_only=True):
        ...
        apply_debug_flags(contiki=join(contiki, 'core', 'net'),
                          new_old_pair=['NONE', 'PRINT'][params["debug"]],
                          logger=logger)
        # add for non-mal
        copy_files(join(path, "templates"), join(path, "without-malicious"),
                   *[(x["dst"], x["dst"]) for x in
                     kwargs["additional_files"]] if "custom_makefile" in kwargs.keys() else [])
        ...
        with lcd(with_malicious):
            ...
            if reuse_bin_path is None or reuse_bin_path == with_malicious:
                ...
                # use stdout to print makefile output
                stderr(local)("make root CONTIKI={} -Wall".format(contiki), capture=True)
                logger.debug(" > Making '{}'...".format(csensor))
                stderr(local)("make sensor CONTIKI={} -Wall".format(contiki), capture=True)
            ...
            apply_replacements(join(contiki, 'core', 'net'), replacements)
            logger.debug(" > Making '{}'...".format(malicious))
            stderr(local)("make malicious CONTIKI={} TARGET={} -Wall"
            ...
            logger.debug(" > Removing malicious.c from {}".format(path))
            # remove_files(with_malicious, 'malicious.c')
            logger.debug(" > Removing folder {}".format(contiki))
...
def __remake(name, build=False, **kwargs):
    ...
    with settings(hide(*HIDDEN_ALL), warn_only=True):
        ...
        with lcd(with_malicious):
            ...
            copy_folder(CONTIKI_FOLDER, with_malicious)
            # includes=get_contiki_includes(params["malicious_target"]))
            if ext_lib is not None:
                remove_folder(contiki_rpl)
                copy_folder(ext_lib, contiki_rpl)
...
def __run(name, **kwargs):
    ...
    with settings(hide(*HIDDEN_ALL), warn_only=True):
        for sim in ["without", "with"]:
            ...
            logger.debug(" > Running simulation {} the malicious mote: {}".format(sim, path))
            task = kwargs.get('task', "run")
            with lcd(sim_path):
                to_run = "make run TASK={}".format(task)
                logger.debug(" > Running shell command: {}".format(to_run))
                output = local(to_run, capture=True)
...
def make_all(exp_file, **kwargs):
    ...
    for name, params in sorted(experiments.items(), key=lambda x: x[0]):
        ...
        if sim_json is not None:
            ...
            # replace position
            motes = relocate_malicious_mote(logger, motes, experiments, current_sim=name)
            params['motes'] = motes
...
def config(contiki_folder='~/contiki', experiments_folder='~/RPL-Attacks-Data', silent=False, **kwargs):
    """
